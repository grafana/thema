...
[^cuevalidity]:
    ...
    Today, lineage authors delegate verification to `BuildLineage()`, resulting in uniform invariant enforcement across all Go `Lineage` instances. But when all necessary constraints are expressed in CUE, verification will shift left (up?). `BuildLineage()` will delegate enforcement to CUE itself, becoming a passthrough more akin to `LineageShip()`, and guarantee uniformity will naturally extend not just to a Go `Lineage`, but to any analogous construct in the Thema bindings for another language (that has a CUE evaluator).
