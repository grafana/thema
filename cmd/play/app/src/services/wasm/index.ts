...
export const ValidateAny = (lineage: string, input: string): string => {
    // @ts-ignore
    const res = validateAny(lineage, input);
    if (res.error !== '') {
        return `'ValidateAny' failed: ${res.error}`;
    }
    return `Input data matches schema version: ${res.result}`;
}

export const ValidateVersion = (lineage: string, input: string, version: string): string => {
    // @ts-ignore
    const res = validateVersion(lineage, input, version);
    if (res.error !== '') {
        return `'ValidateVersion' failed: ${res.error}`;
    }
    return res.result;
}

export const TranslateToLatest = (lineage: string, input: string): string => {
    // @ts-ignore
    const res = translateToLatest(lineage, input);
    if (res.error !== '') {
        return `'TranslateToLatest' failed: ${res.error}`;
    }
    return translateResultToString(res.result);
}

export const TranslateToVersion = (lineage: string, input: string, version: string): string => {
    // @ts-ignore
    const res = translateToLatest(lineage, input);
    if (res.error !== '') {
        return `'TranslateToVersion' failed: ${res.error}`;
    }
    return translateResultToString(res.result);
}
